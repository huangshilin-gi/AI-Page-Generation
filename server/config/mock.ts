 ...
 *
 *   I. 规划阶段 (planning):
 *      - imageAnalysisNode（仅图片输入时执行）, analysisNode, intentNode,
 *        capabilityNode, uiNode, componentNode, structureNode, dependencyNode
 *
 *   II. 基础建设 (foundation):
...
export const MOCK_PRESETS = {
  ...
  /**
   * 仅视觉分析节点用 Mock，其余用真实 LLM
   * 适合调试图片流程、节省 Qwen-VL API 调用
   *
   * ⚠️ 注意：此预设只 Mock 第一个节点（imageAnalysisNode），
   * 后续 intent/ui/代码生成仍走真实 LLM。
   * Mock 数据文件: server/mock/imageAnalysisResult.json
   *
   * 如果希望整条链路都用 Mock（快速看预览效果），请改用 allMock。
   */
  imageAnalysisMock: {
    global: false,
    nodes: { imageAnalysisNode: true },
  } as MockConfig,
} as const;
 ...
 *
 * 示例 - 只 Mock 视觉分析节点:
 *   export const DEFAULT_MOCK_PRESET = MOCK_PRESETS.imageAnalysisMock;
 *
 * 示例 - 手动指定单个节点:
 *   export const DEFAULT_MOCK_PRESET: MockConfig = {
 *     global: false,
 ...
 *   };
 */
export const DEFAULT_MOCK_PRESET: MockConfig = MOCK_PRESETS.allReal;
