...
6. 只能 import【可用组件】中列出的组件，不能凭空导入 Navbar、Header、Footer、Sidebar 等不存在的组件
7. 如果需要导航栏、页脚或侧边栏但可用组件中没有对应组件，请直接在 Layout 组件内部用 JSX 编写
8. 只输出合法 JSON 对象，不要输出 Markdown 代码块或解释文字
`.trim();

  // 4. 调用 LLM。Layout 输出包含大段 TSX 代码
