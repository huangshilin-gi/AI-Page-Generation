...
5. **样式还原优先 (Visual Fidelity First)**:
   - 页面容器必须遵循 UI 架构和设计稿视觉分析中的布局密度、宽度、分栏和背景色。
   - 不要无脑使用 \`max-w-6xl mx-auto p-6\`；如果截图是全宽门户/横向分栏/后台工作区，应使用对应容器结构。
   - 可以使用必要的 Flex/Grid 完成截图中的主布局，但不要过度创造未出现的新结构。
   - 让子组件负责局部样式，页面负责整体区域排列、间距和背景。
