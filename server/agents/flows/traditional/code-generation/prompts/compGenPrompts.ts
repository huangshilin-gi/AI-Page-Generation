...
3. **样式指南**:
   - 风格必须优先遵循组件描述、页面描述和设计稿视觉分析；禁止无视输入统一套用 Linear/Vercel/SaaS 模板风。
   - 只有当输入没有明确视觉风格时，才使用现代、简洁、极简主义作为默认风格。
   - 必须保留输入中体现的主色、背景色、内容密度、圆角/边框风格、导航位置和卡片/列表结构。
   - 新闻门户/官网类页面应更接近信息门户：清晰导航、内容分栏、列表密度适中，而不是后台仪表盘卡片堆叠。
   - 后台仪表盘类页面应保留侧栏、统计卡、表格等结构，不要改成营销首页。
