...
export async function styleGenNode(state: T_Graph) {
  ...
  // 2.4 视觉分析中的配色信息（图片输入时用于提升还原度）
  const imageStyleContext = state.imageAnalysis
    ? `
【设计稿配色（来自视觉分析，请优先使用）】
主色: ${state.imageAnalysis.colorPalette?.primary || "未指定"}
辅助色: ${state.imageAnalysis.colorPalette?.secondary || "未指定"}
背景色: ${state.imageAnalysis.colorPalette?.background || "未指定"}
文字色: ${state.imageAnalysis.colorPalette?.text || "未指定"}
强调色: ${state.imageAnalysis.colorPalette?.accent || "无"}
主题策略: ${state.imageAnalysis.themeStrategy || "未指定"}
字体风格: 标题 ${state.imageAnalysis.typography?.headingStyle || "默认"}，正文 ${state.imageAnalysis.typography?.bodyStyle || "默认"}
`
    : "";
