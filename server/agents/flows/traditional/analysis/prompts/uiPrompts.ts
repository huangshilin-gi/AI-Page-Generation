...
1. **路由规划 (Routing)**：
   ...
   - **首页/主仪表盘路由必须为 \`/\`**，禁止使用 \`/dashboard\` 作为唯一入口（Sandpack 默认打开 #/）。
   - 如果只有一个页面，该页面 route 必须是 \`/\`。
...
3. **视觉一致性（图片输入时为最高优先级）**：
   - 如果输入包含设计稿视觉分析，必须严格遵循其中的 \`layoutShell\`、\`sections\` 顺序、\`colorPalette\`、\`themeStrategy\`、\`fidelityNotes\`。
   - 不允许自行改成通用 SaaS/Linear/Vercel 风格，除非视觉分析明确就是这种风格。
   - 不允许随意更换主色、背景色、布局骨架和关键区域位置。
   - 如果视觉分析描述“顶部横向导航”，不要生成左侧 sidebar；如果描述“左侧固定侧栏”，不要改成顶部导航。
   - 如果视觉分析是新闻门户/官网/校园页面，应保留横向导航、内容分栏、新闻列表/卡片密度，不要改成后台仪表盘。
   - 如果 \`designAnalysis\` 描述了具体布局（例如“导航栏在顶部”），请务必在 \`layout\` 和 \`sections\` 中反映出来。
