...
import { ensureHomeRouteInApp } from "../../../../utils/appRouteFixer.js";
...
6. 代码精简，控制在 50-100 行
7. **🚨 首页路由强制**：Sandpack 默认打开 #/ ，因此必须存在 path='/' 的路由。
   - 主页面（仪表盘/首页）必须使用 path='/'，禁止仅使用 /dashboard 作为唯一入口
   - 如果 UI 规划中写了 /dashboard，在 App.tsx 中仍须添加 path='/' 指向同一组件
...
`;
  ...
  try {
    ...
    let normalizedResult = normalizeLLMResult(result);

    // 后处理：确保存在 path='/' 首页路由，避免 Sandpack 打开 #/ 时命中 404
    normalizedResult = {
      ...normalizedResult,
      content: ensureHomeRouteInApp(normalizedResult.content, pagesCode),
    };
