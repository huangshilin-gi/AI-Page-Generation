...
// Analysis Phase
import { imageAnalysisNode } from "../flows/traditional/image-analysis/nodes/imageAnalysisNode.js";
...
import { hasImageInput } from "../shared/utils/imageHelpers.js";
...
const GraphState = Annotation.Root({
  ...
  // Step -1: 设计稿视觉分析（Qwen-VL）
  imageAnalysis: Annotation<T_Graph["imageAnalysis"]>(),
...
/**
 * 条件路由：根据是否有图片附件决定流程起点
 *
 * - 有图片 → imageAnalysisNode（Qwen-VL 视觉分析）
 * - 无图片 → analysisNode（直接进入文本意图分析）
 */
const routeFromStart = (state: typeof GraphState.State) => {
  if (hasImageInput(state.messages || [])) {
    console.log("[MainGraph] 检测到图片输入，路由至 imageAnalysisNode");
    return "imageAnalysisNode";
  }
  console.log("[MainGraph] 纯文本输入，路由至 analysisNode");
  return "analysisNode";
};

export function buildTraditionalAgent() {
  const builder = new StateGraph(GraphState)
    // Step -1: 设计稿视觉分析（仅图片输入时执行）
    .addNode("imageAnalysisNode", imageAnalysisNode)
    // Step 0: 意图分析节点
    ...
    // 编排流程
    // START 根据是否有图片，条件路由到不同起点
    .addConditionalEdges(START, routeFromStart, {
      imageAnalysisNode: "imageAnalysisNode",
      analysisNode: "analysisNode",
    })
    // 图片分析完成后，进入文本意图分析
    .addEdge("imageAnalysisNode", "analysisNode")
