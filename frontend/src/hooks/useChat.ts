...
export function useChat() {
  ...
  const sendMessage = useCallback(
    async (content: string, attachments?: { type: "image"; url: string }[]) => {
      ...
      // ✨ 初始步骤：有图片附件时从视觉分析开始，否则从需求分析开始
      const hasImages = attachments?.some((a) => a.type === "image");
      const activePhaseTotals = new Map(
        FLOW_CONFIG[flowType].phases.map((phase) => [
          phase,
          PHASE_NODES[phase].filter(
            (node) => node !== "imageAnalysis" || hasImages,
          ).length,
        ]),
      );
      const initialType =
        flowType === "traditional" && hasImages
          ? "imageAnalysis"
          : FLOW_CONFIG[flowType].initialStep;
        ...
        await generateAppStream(
          ...
          (event) => {
            ...
            if (currentPhase) {
              ...
              const activeTotal = activePhaseTotals.get(currentPhase);
              if (
                phaseInfo &&
                activeTotal !== undefined &&
                phaseInfo.completed >= activeTotal
              ) {
                // 阶段完成，触发折叠
