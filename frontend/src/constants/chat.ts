...
export const STEP_DEFINITIONS: Partial<Record<StreamEventType, StepContent>> = {
  ...
  imageAnalysis: {
    title: "设计稿分析",
    description: {
      pending: "正在使用视觉模型分析 UI 设计稿...",
      success: (data: unknown) => {
        const d = data as { pages?: unknown[]; detectedTexts?: string[] };
        const pageCount = d?.pages?.length || 0;
        const textCount = d?.detectedTexts?.length || 0;
        return `视觉分析完成，识别到 ${pageCount} 个页面、${textCount} 条界面文字`;
      },
    },
  },
  analysis: {
